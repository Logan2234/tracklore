<script lang="ts">
	...
	$effect(() => {
		Promise.all([listLibrary({ status: 'WATCHING' }), listLibrary({ status: 'PLANNED' })])
			.then(([w, p]) => {
				watching = w;
				planned = p;
			})
	...
	function pct(e: LibraryEntryDto): number {
		...
		return Math.round((e.progress.watchedEpisodes / e.progress.totalEpisodes) * 100);
	}
...
<div class="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
	...
	<section class="mb-10">
		<div class="mb-4 flex items-baseline justify-between">
			...
			<a href="/library" class="text-sm font-semibold text-dim hover:text-fg">Tout voir →</a>
		</div>
		{#if loading}
			<p class="timecode text-sm">Chargement…</p>
		{:else if watching.length === 0}
			<div class="rounded-xl border border-dashed border-border px-6 py-12 text-center text-dim">
				Rien en cours. <a href="/search" class="font-semibold text-accent hover:underline">Trouve un titre</a> à suivre.
			</div>
		{:else}
			<div class="-mx-4 flex snap-x gap-4 overflow-x-auto px-4 pb-2 md:mx-0 md:px-0">
				{#each watching as e (e.id)}
					<a href={`/library/${e.id}`} class="w-32 shrink-0 snap-start sm:w-36">
						<div class="card overflow-hidden transition-[border-color] hover:border-accent">
							<Poster src={e.mediaItem.posterUrl} title={e.mediaItem.title} />
						</div>
						<p class="mt-2 truncate font-display text-sm font-semibold">{e.mediaItem.title}</p>
						{#if e.progress}
	...
	<section class="mb-10">
		<div class="mb-4 flex items-baseline justify-between">
			<div class="flex items-baseline gap-3">
				...
				<span class="rounded-full bg-surface-2 px-2 py-0.5 text-xs font-semibold text-dim">Aperçu</span>
			</div>
			<a href="/calendar" class="text-sm font-semibold text-dim hover:text-fg">Calendrier →</a>
		</div>
		<div class="grid gap-3 sm:grid-cols-3">
			{#each thisWeek as ep (ep.title + ep.code)}
				<div class="card flex items-center gap-3 p-3">
					...
					<div class="min-w-0">
						<p class="truncate font-display text-sm font-semibold">{ep.title}</p>
