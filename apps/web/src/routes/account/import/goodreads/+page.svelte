<script lang="ts">
  ...
  let preview = $state<GoodreadsImportPreviewDto | null>(null);
  ...
  type Row =
    | { key: string; kind: "matched"; book: GoodreadsMatchedBookDto }
    | { key: string; kind: "unmatched"; book: GoodreadsUnmatchedBookDto };
  ...
  async function analyze() {
    ...
    error = null;
    try {
      const result = await previewGoodreadsImport({ csv });
      preview = result;
  ...
  {#if step === "input"}
    <p class="mb-6 max-w-xl text-sm text-dim">
      <a
        ...
        >Exporte ta bibliothèque depuis Goodreads ↗</a>
