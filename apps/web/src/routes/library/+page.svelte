<script lang="ts">
	...
	function pct(entry: LibraryEntryDto): number {
		...
		return Math.round((entry.progress.watchedEpisodes / entry.progress.totalEpisodes) * 100);
	}
</script>

<div class="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
	<header class="mb-6">
		<h1 class="font-display text-3xl font-extrabold tracking-tight md:text-4xl">Ma bibliothèque</h1>
	...
	<div class="mb-2 flex flex-wrap gap-2">
		{#each STATUS_TABS as tab (tab.label)}
			<button class="chip" class:chip-on={status === tab.value} onclick={() => (status = tab.value)}>
				{tab.label}
			</button>
		{/each}
	</div>
	<div class="mb-7 flex flex-wrap gap-2">
		{#each TYPE_TABS as tab (tab.label)}
			<button class="chip" class:chip-on={type === tab.value} onclick={() => (type = tab.value)}>
				{tab.label}
			</button>
		{/each}
	</div>
	...
	{#if error}
		<p class="rounded-lg border border-danger/40 bg-danger/10 px-4 py-3 text-sm text-danger">{error}</p>
	{:else if loading}
				...
				>
					...
					<div class="flex flex-col gap-1.5 p-3">
						<span class="font-display text-sm leading-tight font-semibold">
							{#if entry.favorite}<span class="text-accent">★</span>&nbsp;{/if}{entry.mediaItem.title}
						</span>
						{#if entry.progress}
