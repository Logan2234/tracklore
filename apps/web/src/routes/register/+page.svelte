...
<div class="flex min-h-screen items-center justify-center px-4 py-12">
	<div class="w-full max-w-sm">
		<div class="mb-8 text-center">
			...
			<p class="mt-2 text-sm text-dim">Suis tout ce que tu regardes, au même endroit.</p>
		</div>

		<form onsubmit={submit} class="card flex flex-col gap-4 p-7">
			<h1 class="font-display text-xl font-bold">Créer un compte</h1>
			<input type="text" placeholder="Pseudo" bind:value={displayName} required class="input" />
			...
			<p class="text-center text-sm text-dim">
				Déjà inscrit ?
				<a href="/login" class="font-semibold text-accent hover:underline">Se connecter</a>
			</p>
		</form>
	</div>
</div>
