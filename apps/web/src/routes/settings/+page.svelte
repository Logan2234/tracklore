<script lang="ts">
	...
	import { ApiError, getTvTimeImportJob, logout, startTvTimeImport } from '$lib/api/client';
	...
	async function startImport() {
		...
		try {
			job = await startTvTimeImport({ zipBase64, dryRun, importMovies, overwrite });
			if (job.status === 'running') void pollJob(job.id);
		} catch (err) {
			error = err instanceof ApiError ? err.message : 'Impossible de démarrer l’import.';
		} finally {
			isSubmitting = false;
		}
	}
	...
	async function pollJob(jobId: string) {
		try {
			...
			error = err instanceof Error ? err.message : 'Impossible de récupérer l’état du job.';
		}
	}

	// --- Derived display helpers ---
	const progressTotal = $derived(job ? job.progress.totalShows + job.progress.totalMovies : 0);
	const progressDone = $derived(job ? job.progress.shows + job.progress.movies : 0);
...
<div class="mx-auto max-w-3xl px-4 py-6 md:px-8 md:py-10">
	<h1 class="mb-6 font-display text-3xl font-extrabold tracking-tight md:text-4xl">Réglages</h1>
	...
	{#if auth.user}
		...
		<section class="card p-5 md:p-6">
			...
			<p class="mb-5 text-sm text-dim">
				Dépose l’archive <code class="rounded bg-bg px-1.5 py-0.5 font-mono text-xs">.zip</code> de ton
			...
			>
				...
				{#if zipBase64}
					...
					<span class="timecode text-sm">{formatBytes(fileSize)} · clique pour changer</span>
				{:else}
			...
			<div class="my-5 flex flex-col gap-3">
				<label class="flex items-center gap-2.5">
					<input type="checkbox" bind:checked={importMovies} class="accent-[var(--accent)]" />
					Importer les films
				</label>
				<label class="flex items-center gap-2.5">
					<input type="checkbox" bind:checked={dryRun} class="accent-[var(--accent)]" />
					Mode aperçu <span class="text-sm text-dim">(analyse sans rien écrire)</span>
				</label>
				<label class="flex items-center gap-2.5 text-danger">
					<input type="checkbox" bind:checked={overwrite} class="accent-[var(--danger)]" />
					Écraser toutes mes données
					<span class="text-sm opacity-80">(sinon l’import s’ajoute à l’existant)</span>
				</label>
			</div>

			{#if overwrite && !dryRun}
				<p class="mb-4 rounded-lg border border-danger bg-danger/10 px-3 py-2.5 text-sm text-danger">
			...
			>
				{isSubmitting ? 'Démarrage…' : dryRun ? 'Analyser l’archive' : 'Démarrer l’import'}
			</button>
				...
				<div class="mt-6">
					<div class="mb-4 flex items-center gap-3">
						...
						{#if r.overwrite && !r.dryRun}
							<span class="rounded-full bg-danger/15 px-2.5 py-0.5 text-xs font-semibold text-danger">
								Données remplacées
							</span>
						{/if}
					</div>
					...
					<div class="grid gap-3 sm:grid-cols-3">
						<div class="rounded-lg border border-border bg-bg p-4">
							...
							<p class="font-display text-2xl font-bold">
								{r.shows.imported}<span class="text-sm font-normal text-dim"> importées</span>
							</p>
						...
						<div class="rounded-lg border border-border bg-bg p-4">
							...
							<p class="font-display text-2xl font-bold">
								{r.episodes.watched}<span class="text-sm font-normal text-dim"> vus</span>
							</p>
							<ul class="mt-1 text-sm">
						...
						<div class="rounded-lg border border-border bg-bg p-4">
							...
							<p class="font-display text-2xl font-bold">
								{r.movies.imported}<span class="text-sm font-normal text-dim"> importés</span>
							</p>
							<ul class="mt-1 text-sm">
					...
					{#if r.shows.unresolved.length || r.episodes.unmatched.length || r.movies.unresolved.length}
						<div class="mt-5 flex flex-col gap-2.5">
							<p class="text-sm text-dim">
								Éléments non concluants — échantillon (100 max). Une étape de réconciliation manuelle
								viendra plus tard.
							</p>

							{#if r.shows.unresolved.length}
								<details class="rounded-lg border border-border bg-bg px-4 py-3">
									<summary class="cursor-pointer font-semibold">
										Séries non résolues ({r.shows.unresolved.length})
									</summary>
									<div class="mt-3 flex max-h-64 flex-col gap-1.5 overflow-y-auto">
											...
											<details class="rounded-md border border-border bg-surface px-3 py-2">
												...
												{#if show.episodes.length > 0}
													<div class="mt-2 flex flex-wrap gap-1.5">
														{#each sortEpisodes(show.episodes) as ep}
															<span class="timecode rounded bg-surface-2 px-1.5 py-0.5 text-xs">
												...
												{:else}
													<p class="mt-2 text-sm text-dim">
														En watchlist : jamais commencée (aucun épisode vu dans l’export).
													</p>
												{/if}
											</details>
										{/each}
							...
							{#if r.movies.unresolved.length}
								<details class="rounded-lg border border-border bg-bg px-4 py-3">
									...
									<ul class="mt-3 flex max-h-64 flex-col gap-1.5 overflow-y-auto text-sm">
										{#each r.movies.unresolved as movie}
											<li class="flex items-baseline gap-2">
												...
												{movie.title}{#if movie.year}<span class="text-dim"> · {movie.year}</span>{/if}
											</li>
										{/each}
									</ul>
								</details>
