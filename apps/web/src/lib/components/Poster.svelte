...
{#if src}
	<img {src} alt={title} loading="lazy" class="aspect-[2/3] w-full bg-surface-2 object-cover {cls}" />
{:else}
	<div
		class="flex aspect-[2/3] w-full items-end p-2 {cls}"
		style="background: linear-gradient(150deg, hsl({hue} 32% 24%), hsl({(hue + 40) % 360} 34% 13%));"
	>
		<span
