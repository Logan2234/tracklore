...
async function request<T>(path: string, options: RequestOptions = {}, retried = false): Promise<T> {
  ...
  if (response.status === 401 && options.withAuth !== false && !retried && auth.refreshToken) {
...
async function tryRefresh(): Promise<boolean> {
  try {
    const { tokens } = await request<{ tokens: AuthTokensDto }>(
      '/auth/refresh',
      { method: 'POST', body: { refreshToken: auth.refreshToken }, withAuth: false },
      true,
    );
...
export async function register(body: RegisterRequestDto): Promise<void> {
  const result = await request<{ user: UserDto; tokens: AuthTokensDto }>('/auth/register', {
    method: 'POST',
    body,
    withAuth: false,
  });
...
export async function login(body: LoginRequestDto): Promise<void> {
  const result = await request<{ user: UserDto; tokens: AuthTokensDto }>('/auth/login', {
    method: 'POST',
    body,
    withAuth: false,
  });
...
export function searchCatalog(query: string, type?: MediaType): Promise<SearchResponseDto> {
...
export function upsertLibraryEntry(body: UpsertLibraryEntryDto): Promise<LibraryEntryDto> {
...
export function updateLibraryEntry(
  entryId: string,
  body: Partial<Pick<LibraryEntryDto, 'status' | 'rating' | 'notes' | 'favorite' | 'archived'>>,
): Promise<LibraryEntryDto> {
...
export function getEntryEpisodes(entryId: string): Promise<EntryEpisodesResponseDto> {
  return request(`/library/entries/${entryId}/episodes`);
}

export function watchEpisode(episodeId: string, rating?: number): Promise<EpisodeWatchDto> {
